with bif as (select max(thread_name) as bif_name from biffer),
	likes_r as (select distinct p.thread_name, p.user_id, count(*) as likes_received from post p join "like" l on p.id = l.post_id group by 1,2),
	likes_g as (select distinct p.thread_name, l.user_id, count(*) as likes_given from "like" l join post p on l.post_id = p.id group by 1,2),
	likes_gifs as (select distinct p.thread_name, p.user_id, count(*) as likes_from_gifs from post p join "like" l on p.id = l.post_id where gifs > 0 group by 1,2),
	likes_posts as (select p.thread_name, count(distinct l.post_id) as posts_with_likes from "like" l join post p on l.post_id = p.id group by 1),
	final as (select distinct u.username, p.thread_name as thread, case when u.id in (select user_id from biffer where thread_name = (select distinct bif_name from bif)) then 'Biffer' else 'Wanker' end as participant,
