...
import sqlite3, re, html, datetime, requests, dateutil.parser
...
def elkhunter(data, name, postinfo):
    # validation
    if data == None: stop("No data returned")
    ...
    for d in data:
        ...
        # remove quoted posts
        while soup.find('div', class_="bbCodeBlock bbCodeQuote") != None:
...
def update_file(name):
    ...
    if soup.find('a', title="Open quick navigation") != None: soup.find('a', title="Open quick navigation").decompose()
    if soup.find('header') != None: soup.find('header').decompose()
    if soup.find('div', class_="quickReply message") != None: soup.find('div', class_="quickReply message").decompose()
    if soup.find('div', id="loginBar") != None: soup.find('div', id="loginBar").decompose()
    ...
    for d in data:
        ...
        remove.append(str(sp.find('div', class_="messageMeta ToggleTriggerAnchor"))) #user & timestamp summary after the post
    ...
    # write the output
    with open(name + ' ' + options[option] + '.html', 'w') as file: file.write(html)
...
def update_likes(name):
    ...
    global login_status
    if login_status == False and True==False:
    ...
    tbdb.execute('SELECT post_id, max(timestamp) FROM likes WHERE post_id = (SELECT max(l.post_id) as maxpost FROM likes l JOIN posts p on l.post_id = p.id WHERE p.thread_name = ?) GROUP BY post_id;', (name,))
    ...
    try:
        if int(starting_post) > 180: #user entered a post_id
            tbdb.execute('SELECT distinct id, timestamp FROM posts where id >= ? and thread_name = ? ORDER BY id LIMIT 1500', (starting_post, name))
        else: #user entered a number of days
            cutoff_date = str(datetime.datetime.now() + datetime.timedelta(-starting_post))[:10]
            tbdb.execute('SELECT distinct id, timestamp FROM posts where timestamp >= ? and thread_name = ? ORDER BY id LIMIT 1500', (cutoff_date, name))
    except:
        if '-' in starting_post: #user entered a timestamp
            tbdb.execute('SELECT distinct id, timestamp FROM posts where timestamp >= ? and thread_name = ? ORDER BY id LIMIT 1500', (starting_post, name))
        else: return 0
    ...
    tbdb.execute('SELECT distinct user_id FROM likes WHERE user_id not in (SELECT distinct id FROM users) ORDER BY user_id')
    users = tbdb.fetchall()
    print(users, "||", type(users))
    if users == None or len(users) < 1: return 0
...
def determine_thread(): #returns the thread name and the highest page number that's been parsed
    ...
    if to_agg == None or len(to_agg) != 1: #prompt user for the thread to parse
    ...
    if lp_threads == None: stop("No thread data for " + name)
    elif lp_threads == lp_posts: stop("No new posts found for " + name)
    elif lp_posts == None: page = 1 #no post data recorded for this thread yet
...
def thumbfix(p):
    ...
    if thumbs == None or thumbs == []: return p
...
for d in data:
    ...
    for p in posts: #loop through each post on this page
        ...
        while i < len(pics): #iterate through the pics
            ...
            if 'data-url' not in pic_blob and 'src="https://www.talkbeer.com/community/attachments/' not in pic_blob: #tb-native pics are stored differently
                i += 1
                continue
            ...
            p = p.replace(pic_blob, pic_link + '\n' + '<div id="pic_blob">' + pic_blob + '</div>') #p is still a string here
        ...
        while m < len(media):
            ...
            # youtube videos are an iframe within a distinct <span>; instagram posts are entirely enclosed in an iframe, no <span>; gfycat embeds are a mix
            ...
            try: media_type = re.findall('mediaembed=\"(.+?)\"', media_blob)[0]
                ...
                elif 'src="/' in media_blob: media_type = re.findall('src=\".*\W([0-z]+)\.[0-z]+?/', media_blob)[0] #uses the domain name with //domain.com/ prefix
                else: media_type = re.findall('src=\".*\.(.+?)\.', media_blob)[0] #uses the domain name as the media type
        ...
        #print("*=*=*=*=*=*=*=*=*=* MESSAGE *=*=*=*=*=*=*=*=*=*")
        #print(",", "Post: #" + str(post_num) + ",", "gifs:", str(gifs) + ",", "Pics:", str(pic_counter) + ",", "Media:", str(media_counter) + ",", "Hint:", str(hint) + ",", "Timestamp:", timestamp, "Username:", username)
        #pp("")
